...
from aps.wavefront_analysis.spinnet.speckle_displacement.DS_network import MODULE_PATH
# Insert the absolute path to the directory containing the DS_network module to the system path
sys.path.insert(0, MODULE_PATH)

from aps.wavefront_analysis.spinnet.speckle_displacement.DS_network import SpeckleDisplacementNet
...
class args_setting:
    def __init__(self, dict_para):
        ...
        self.device = dict_para['nnp_parameters']['device']
...
def estimate(img_stack, model, args):
    ...
    with torch.no_grad():
        # Transfer to GPU
        img_stack = img_stack.to(args.device)
...
def SPINNet_estimate(ref, img, model_path, setting_path, device):
    ...
    with open(setting_path) as f: setting_dict = json.load(f)

    setting_dict['nnp_parameters']['device'] = device

    args = args_setting(setting_dict)

    torch.manual_seed(42)
    # model = Network(args).to(args.device)
    model = SpeckleDisplacementNet().to(args.device)
